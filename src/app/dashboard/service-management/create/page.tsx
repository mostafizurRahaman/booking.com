            ...
            {facilitesEnum.map((data, index) => (
              <div className=" text-sm pb-1" key={index}>
                <input
                  ...
                  onClick={(e)=> {
                    const isExit = formData?.facilities?.find((item:string) => item ===  data); 
                    if(!isExit){
                        setFormData({...formData, facilities: [...formData.facilities, data]})
                    }else{
                        const rest = formData?.facilities.filter((item:string)  => item !== data);
